            ...
            <section class="user-detail max-width">
                <section class="header-user-detail">
                    ...
                    <section class="user-detail-title">
                        <h1>{{ trans('message.add_course') }}</h1>
                       ...
                       <div class="input-container">
                            <section class="user-detail-row">
                                <div class="input-container">
                                    ...
                                    <div>
                                        <form action="{{ route('profile.store') }}" method="POST" id="update-email-form" enctype="multipart/form-data">
                                            @csrf
                                            <input type="text" maxlength="50" name="name" autocomplete="off" placeholder="{{ trans('message.type_course_name') }}" class="@error('name') error-focus @enderror">
                                            @error('name')
                                                <p class="error-message">{{ $message }}</p>
                                            @enderror

                                            <label for class="label-add-course">{{ trans('message.description') }}</label>
                                            <textarea type="text" cols="60" rows="5" autocomplete="off" name="description" placeholder="{{ trans('message.type_course_name') }}" class="@error('description') error-focus @enderror"></textarea>
                                            @error('description')
                                                <p class="error-message">{{ $message }}</p>
                                            @enderror
